...
import { subscriptionAccess } from './subscription'
...
export async function deleteContract(cont_id, email) {
    try {
        ...
        .eq('cont_id', cont_id)
        .eq('group_id', userCheck.group_id)
        .single()

        if(checkError) throw checkError
        if(!contract) throw new Error("Contract not found")
        if(contract.uploaded_by !== userCheck.user_id && !userCheck.admin) throw new Error("Unauthorized to delete this contract")

        const { data: contractDates, error: fetchError } = await supabaseAdmin
        ...
        // Extract all unique date IDs
        const dateIds = [...new Set(contractDates.map(item => item.date_id))];

        const operations = [
            // Delete contract dates references
            supabaseAdmin.from('contracts_dates').delete().eq('cont_id', cont_id),
            // Delete contract metadata
            supabaseAdmin.from('contracts_metadata').delete().eq('cont_id', cont_id),
            // Delete main contract record
            supabaseAdmin.from('contracts').delete().eq('cont_id', cont_id),
            // Delete from storage
            supabaseAdmin.storage.from('contracts').remove([contract.file_path])
        ];

        // Delete contract dates
        if (dateIds.length > 0) {
            operations.push(supabaseAdmin.from('dates').delete().in('date_id', dateIds));
        }
        // Execute all operations
        const results = await Promise.allSettled(operations);
        
        // Check for errors (excluding storage errors which might be ok if file doesn't exist)
        const errors = results
            .map((result, index) => ({
                operation: ['dates', 'metadata', 'contract', 'storage'][index],
                error: result.status === 'rejected' ? result.reason : null
            }))
            .filter(item => item.error);

        if (errors.length > 0) {
            throw new Error(`Deletion partially failed: ${errors.map(e => e.operation).join(', ')}`);
        }
